...
PRESETS = {
    "⚡ Speed (4 steps — 6.7× faster)": {
        ...
        "benchmark": {"latency": "~0.876s", "speedup": "6.5×", "vram": "13.4 GB", "clip": "0.327"},
    },
    "⚖️ Balanced (8 steps — 5.5× faster, 25% less VRAM)": {
        ...
        "benchmark": {"latency": "~1.26s", "speedup": "4.5×", "vram": "9.7 GB", "clip": "0.322"},
    },
    "🧮 HQQ 4-bit + DeepCache (1.7× faster, 11% less VRAM)": {
        "config": CompressionConfig(
            name="hqq", dtype="fp16",
        ...
        "benchmark": {"latency": "~3.31s", "speedup": "1.7×", "vram": "11.6 GB", "clip": "0.318"},
    },
...
def generate(prompt, preset_name, seed):
    ...
    metrics = (
        ...
        f"| **Speedup vs baseline** | **{5.72 / latency:.1f}×** |\n"
...
def compare(prompt, seed):
    ...
    comparison = (
        ...
        f"| **Speedup** | **{5.72/results[0][1]:.1f}×** | {5.72/results[1][1]:.1f}× |\n"
...
with gr.Blocks(
    ...
    gr.Markdown(
        ...
        "Systematic optimization of Stable Diffusion XL — from **5.7s → 0.876s** per image (6.5× speedup).\n\n"
    ...
    with gr.Tab("📊 Benchmark Results"):
        gr.Markdown(
            ...
            "|---|---|---|---|---|---|\n"
            "| Baseline (50 steps) | 5.72s | 1.0× | 13.0 GB | 0.322 | Reference |\n"
            "| INT8 UNet (BitsAndBytes) | 33.1s | 0.17× | 10.8 GB | 0.324 | ❌ Slower — dequant overhead |\n"
            "| NF4 UNet (BitsAndBytes) | 16.0s | 0.36× | 9.6 GB | 0.324 | ❌ Slower — dequant overhead |\n"
            "| **HQQ 4-bit** | **5.65s** | **1.0×** | **11.3 GB** | **0.317** | ✅ Same speed, 13% less VRAM |\n"
            "| **HQQ 4-bit + DeepCache** | **3.31s** | **1.7×** | **11.6 GB** | **0.318** | ✅ |\n"
            "| **HQQ + DeepCache (pruna)** | **5.07s** | **1.1×** | **9.5 GB** | **0.323** | ✅ 27% less VRAM |\n"
            "| DeepCache (N=2) | 3.40s | 1.7× | 13.4 GB | 0.326 | ✅ |\n"
            "| DeepCache (N=3) | 2.48s | 2.3× | 13.4 GB | 0.324 | ✅ |\n"
            "| **LCM 4-step** | **0.876s** | **6.5×** | 13.4 GB | **0.327** | ✅ Best speed |\n"
            "| **LCM + Compile + TinyVAE** | **1.26s** | **4.5×** | **9.7 GB** | 0.322 | ✅ Best balanced |\n"
            "| torch.compile | 35.9s | 0.16× | 13.0 GB | 0.322 | ⚠️ Std=43s — huge warmup variance |\n"
            "| Tiny VAE | 5.56s | 1.0× | 9.3 GB | 0.328 | ✅ Memory only |\n"
            "| LCM + DeepCache | 0.891s | 6.4× | 13.8 GB | 0.208 | ❌ Quality collapse |\n"
            "| Full stack (no compile) | 1.51s | 3.8× | 6.7 GB | 0.128 | ❌ Garbage output |\n"
            "\n### Why BitsAndBytes was slower but HQQ is different\n"
            "- **BitsAndBytes** dequantizes to FP16 before every matmul — overhead dominates on A100\n"
            "- **HQQ 4-bit** matches baseline speed with 13% less VRAM — on VRAM-constrained GPUs this is the right pick\n"
            "- **HQQ + DeepCache** reaches 1.7× speedup + 11% VRAM reduction\n"
            "- **HQQ + DeepCache via pruna library** achieves 27% VRAM reduction (9.5 GB vs 13.0 GB baseline)\n"
            "\n### Key Takeaways\n"
            "- **Best speed:** LCM 4-step — 6.5× faster, same CLIP score\n"
            "- **Best balanced:** LCM + compile + TinyVAE — 4.5× faster, 25% less VRAM\n"
            "- **Best for VRAM-constrained:** HQQ 4-bit + DeepCache — 9.5 GB, near-baseline quality\n"
            "- **Not all axes compose** — LCM+DeepCache (CLIP 0.208) and full_stack (CLIP 0.128) both failed\n"
            "- **torch.compile** std=43s — useless for low-batch inference without many warmup runs"
        )
